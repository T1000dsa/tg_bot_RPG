...
class HostileAction:
    ...
    def attack(self) -> dict[str, int]:
        ...
        if enemy_hp > 0 and enemy_def_mag == 0:
            enemy_hp-=player_att_mag

        if enemy_hp > 0 and enemy_def_phys == 0:
            enemy_hp-=player_att_phys
        ...
        summarize = 0
        if enemy_def_phys > 0:
            if player_att_phys//diff > enemy_def_phys:
                summarize = player_att_phys//diff-enemy_def_phys
                enemy_def_phys = 0
                summarize*=diff
            
            elif player_att_phys//diff <= enemy_def_phys: # 10//2 = 5 <= 10 5-10 = -5 
                summarize = player_att_phys//diff-enemy_def_phys
        ...
        summarize = 0
        if enemy_def_mag > 0:
            if player_att_mag//diff > enemy_def_mag:
                summarize = player_att_mag//diff-enemy_def_mag
                enemy_def_mag = 0
                summarize*=diff
                
            elif player_att_mag//diff <= enemy_def_mag:
                summarize = player_att_mag//diff-enemy_def_mag
...
class DefenseAction:
    ...
    def defense(self) -> dict[str, int]:
        ...
        if player_hp > 0 and player_def_mag == 0:
            player_hp-=enemy_att_mag

        if player_hp > 0 and player_def_phys == 0:
            player_hp-=enemy_att_phys
        ...
        if player_def_phys > 0:
            if enemy_att_phys//diff > player_def_phys:
                summarize = enemy_att_phys//diff-player_def_phys
                player_def_phys = 0
                summarize*=diff
            
            elif enemy_att_phys//diff <= player_def_phys: # 10 <= 20 10-20 = -10 
                summarize = enemy_att_phys//diff-player_def_phys
        ...
        summarize = 0
        if player_def_mag > 0:
            if enemy_att_mag//diff > player_def_mag:
                summarize = enemy_att_mag//diff-player_def_mag
                player_def_mag = 0
                summarize*=diff
                
            elif enemy_att_mag//diff <= player_def_mag:
                summarize = enemy_att_mag//diff-player_def_mag
